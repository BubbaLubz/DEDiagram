...
function CustomBoxEditor({ nodeId, initialLabel, initialNotes, initialIcon }) {
  ...
  return (
    <div className="space-y-4">
      {/* Icon picker */}
      <div>
        <p className="text-xs font-semibold uppercase tracking-wider mb-2 text-slate-400">Icon</p>
        <button
          onClick={() => setPickerOpen(o => !o)}
          title="Click to change icon"
          className="w-12 h-12 rounded-xl flex items-center justify-center text-2xl border border-slate-700 bg-slate-900 hover:border-indigo-500 transition-colors"
        >
          {icon}
        </button>
        {pickerOpen && (
          <div className="mt-2 p-2 rounded-xl border border-slate-700 bg-slate-900 grid grid-cols-8 gap-0.5">
            {EMOJI_OPTIONS.map(emoji => (
              <button
                key={emoji}
                onClick={() => handleIconSelect(emoji)}
                className={`w-8 h-8 flex items-center justify-center text-lg rounded hover:bg-slate-700 transition-colors ${icon === emoji ? 'bg-slate-700 ring-1 ring-indigo-500' : ''}`}
              >
                {emoji}
              </button>
            ))}
      ...
      {/* Name */}
      <div>
        <p className="text-xs font-semibold uppercase tracking-wider mb-2 text-slate-400">Name</p>
        <input
          value={label}
          onChange={(e) => { setLabel(e.target.value); updateNodeData(nodeId, { label: e.target.value }); }}
          className="w-full px-3 py-2 rounded-lg text-sm text-slate-100 border border-slate-700 bg-slate-900 focus:outline-none focus:border-indigo-500 transition-colors"
      ...
      {/* Description */}
      <div>
        <p className="text-xs font-semibold uppercase tracking-wider mb-2 text-slate-400">Description</p>
        <textarea
          value={notes}
          onChange={(e) => { setNotes(e.target.value); updateNodeData(nodeId, { notes: e.target.value }); }}
          rows={4}
          className="w-full px-3 py-2 rounded-lg text-sm text-slate-100 border border-slate-700 bg-slate-900 focus:outline-none focus:border-indigo-500 transition-colors resize-none"
